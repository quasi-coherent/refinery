...
use quote::ToTokens;
use refinery_core::{
    find_migration_files, parse_finalize_migration, parse_no_transaction, FinalizeType,
    MigrationType,
};
...
fn finalize_migration_fn_quoted<T: ToTokens>(
    _finalized_migrations: Vec<T>,
    finalize_runtime: Option<FinalizeType>,
) -> TokenStream2 {
    let result = match finalize_runtime {
        None => quote!(),
        Some(FinalizeType::Sync) => {
            quote! {
...
fn unapplied_migration_quoted(
    filename: &str,
    no_transaction: Option<bool>,
    finalize_type: Option<FinalizeType>,
) -> TokenStream2 {
    ...
    match finalize_type {
        // This is a Rust migration with no finalize annotation
        None => {
            quote! {Migration::unapplied(#filename, #no_transaction_token, #ident::migration()).unwrap()}
        }
        Some(FinalizeType::Async) => {
            quote! {Migration::async_finalize_unapplied::<#ident::Finalizer, _>(conn, #filename, #no_transaction_token).unwrap()}
        }
        Some(FinalizeType::Sync) => {
            quote! {Migration::finalize_unapplied::<#ident::Finalizer, _>(conn, #filename, #no_transaction_token).unwrap()}
        }
    }
}
...
pub fn embed_migrations(input: TokenStream) -> TokenStream {
    ...
    let mut finalize_runtime: Option<FinalizeType> = None;
    ...
    for migration in migration_files {
        ...
        if extension == "sql" {
            ...
            let ident = Ident::new(&filename, Span2::call_site());
            let finalize_type = parse_finalize_migration(rs_raw.to_string());
            match (finalize_type, finalize_runtime) {
                (Some(ann), Some(rt)) if ann != rt => {
                    panic!("migration annotated for {:?} finalizer but another migration has a different runtime", ann);
                }
                (Some(ann), None) => finalize_runtime = Some(ann),
                _ => (),
            };
            ...
            let rs_mig = unapplied_migration_quoted(&filename, no_transaction, finalize_type);
